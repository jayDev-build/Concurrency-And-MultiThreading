...
Where t2 lives: When you typed MyThread t2 = new MyThread("Task-2");, 
...
t2 on its private Stack. When you call t2.run(), the main thread simply uses that 
...
t1.start(): You (the main thread) hire a new worker (Thread-0), point to blender t1, and say, 
"Go turn that on." The worker goes and does it in the background while you keep moving.<br>

t2.run(): You (the main thread) walk up to blender t2 and turn it on yourself. 
